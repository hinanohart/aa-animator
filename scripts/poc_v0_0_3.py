...
OUTPUT_MP4  = os.path.join(OUT_DIR, f"poc_v4_output_{BG_MODE}_{'braille' if BRAILLE_ON else 'ascii'}.mp4")
FRAME0_PNG  = os.path.join(OUT_DIR, f"poc_v4_frame0_{BG_MODE}_{'braille' if BRAILLE_ON else 'ascii'}.png")
...
def make_synthetic(out_path: str) -> Image.Image:
    ...
    draw.ellipse([cx - 60, cy - 60, cx + 60, cy + 60], fill=(80, 120, 255), outline=(255, 255, 255), width=2)
...
# ── 1. Load input image ──────────────────────────────────────────────
stamp("start")
print(f"[{now():.3f}s] aa-animator v0.0.3  BG_MODE={BG_MODE}  BRAILLE={'on' if BRAILLE_ON else 'off'}")
    ...
    if os.path.isfile(MASK_CACHE):
        ...
        try:
            ...
            if coverage_rembg < 10.0:
                print(f"  [WARN] rembg coverage too low ({coverage_rembg:.1f}%), using luminance Otsu fallback")
                gray_np = np.array(img.convert("L"), dtype=np.float32)
                lum_otsu = _otsu_threshold(gray_np.astype(np.uint8))
                fg_mask = gray_np > lum_otsu
                print(f"  Luminance Otsu thresh={lum_otsu:.0f}  coverage={fg_mask.mean()*100:.1f}%")
            else:
...
for f in range(N_FRAMES):
    ...
    gray_full = (0.299 * warped[:, :, 0] +
                 0.587 * warped[:, :, 1] +
                 0.114 * warped[:, :, 2])

    cell_brightness = (gray_full
    ...
    if warped_mask is not None:
        mask_cell = (warped_mask
                     .reshape(ROWS, CELL_H, COLS, CELL_W)
                     .mean(axis=(1, 3)) > 0.3)
    else:
        mask_cell = np.ones((ROWS, COLS), dtype=bool)

    rgb_cell_mean = (warped
    ...
    if BRAILLE_ON:
        ...
        canvas_u8 = np.ascontiguousarray(
    ...
    if BG_MODE == "black":
        ...
        bg_canvas = np.tile(
            _bg_dot_bitmaps[np.newaxis, np.newaxis, :, :, :],
            (ROWS, COLS, 1, 1, 1)
        ).transpose(0, 2, 1, 3, 4).reshape(CANVAS_H, CANVAS_W, 3)
    ...
    if glow_active.any():
        ...
        glow_region  = glow_region * (1.0 - GLOW_ALPHA) + np.array(GLOW_COLOR, dtype=np.float32) * GLOW_ALPHA
    ...
    if (f + 1) % 10 == 0 or f == 0:
        print(f"  Frame {f+1:2d}/{N_FRAMES}: warp={warp_times[-1]*1000:.1f}ms  AA={aa_times[-1]*1000:.1f}ms  fg_ent={ent_fg:.3f}")
...
print(f"  Full entropy avg  : {avg_entropy:.3f} bits (max {max_ent_bits:.3f})  [{ok(entropy_ok)} >=3.0]")
print(f"  fg_entropy avg    : {avg_fg_entropy:.3f} bits (foreground only)  [{ok(fg_entropy_ok)} >=3.0]")
