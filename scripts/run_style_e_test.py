...
"""Style E signal-driven animation test runner.

Generates 3 images × 5 signals = 15 MP4s in /tmp/aa_v03_signal/,
then sends all 15 to Discord with labelled messages.

Usage:
    python3 run_style_e_test.py [--dry-run]
"""
...
_OUT_DIR = Path("/tmp/aa_v03_signal")
...
_SIGNALS: list[str] = list(VALID_SIGNALS)  # jump, scan, wave, pulse, combo
...
def main() -> None:
    ...
    total = len(_IMAGES) * len(_SIGNALS)
    done = 0
    ...
    for img_path, img_name in _IMAGES:
        ...
        for sig in _SIGNALS:
            ...
            out_name = f"styleE_{sig}_{img_name}.mp4"
            out_path = _OUT_DIR / out_name
            label = f"aa-animator v0.3 Style E signal:{sig} - {img_name}"

            print(f"[{done}/{total}] {label}")
            t0 = time.time()

            try:
                info = generate_style_e(
                    img_path,
                    out_path,
                    signal=sig,
                    cols=args.cols,
                ...
                print(
                    ...
                    f"char_grid_invariant={info['char_grid_invariant']}"
                )
                rec = {
                    "output": str(out_path),
                    "signal": sig,
